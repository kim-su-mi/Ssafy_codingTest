	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
                ...
                if(order == 1) {
                    // 선공: 1번, 3번, 8번(5번 인덱스) 공을 순서대로 맞춰야 함
                    if(balls[1][0] != -1 && balls[1][1] != -1) {  // 1번공이 아직 남아있으면
                        isTarget[1] = true;
                    }
                    else if(balls[3][0] != -1 && balls[3][1] != -1) {  // 3번공이 아직 남아있으면
                        isTarget[3] = true;
                    }
                    else if(balls[5][0] != -1 && balls[5][1] != -1) {  // 8번공이 아직 남아있으면
                        isTarget[5] = true;
                    }
                } else {
                    // 후공: 2번, 4번, 8번(5번 인덱스) 공을 순서대로 맞춰야 함
                    if(balls[2][0] != -1 && balls[2][1] != -1) {  // 2번공이 아직 남아있으면
                        isTarget[2] = true;
                    }
                    else if(balls[4][0] != -1 && balls[4][1] != -1) {  // 4번공이 아직 남아있으면
                        isTarget[4] = true;
                    }
                    else if(balls[5][0] != -1 && balls[5][1] != -1) {  // 8번공이 아직 남아있으면
                        isTarget[5] = true;
